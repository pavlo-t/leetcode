...
impl Solution {
    ...
    pub fn max_result_dequeue_and_dp(nums: Vec<i32>, k: i32) -> i32 {
        let n = nums.len();
        let k = k as usize;
        ...
        for i in 1..n {
            while !dq.is_empty() && i >= k && dq.front().unwrap() < &(i - k) { dq.pop_front(); }
            dp[i] = dp[*dq.front().unwrap()] + nums[i];
            while !dq.is_empty() && dp[*dq.back().unwrap()] <= dp[i] { dq.pop_back(); }
            dq.push_back(i);
        }
    ...
    pub fn max_result_2_vectors(nums: Vec<i32>, k: i32) -> i32 {
        let n = nums.len();
        let k = k as usize;
        ...
        for i in 1..n {
            while l <= r && i >= k && q[l] < i - k { l += 1; }
            dp[i] = dp[q[l]] + nums[i];
            while l <= r && dp[i] > dp[q[r]] { r -= 1; }
            r += 1;
            q[r] = i;
        }
    ...
    pub fn max_result_my_dp2(nums: Vec<i32>, k: i32) -> i32 {
        let n = nums.len();
        let k = k as usize;

        let mut dp = vec![*nums.last().unwrap(); k];

        for i in (0..n - 1).rev() {
            dp[i % k] = nums[i] + dp.iter().max().unwrap();
        }

        dp[0]
    }

    pub fn max_result_my_dp1(nums: Vec<i32>, k: i32) -> i32 {
        let n = nums.len();
        let k = k as usize;
        ...
        for i in n - 1..n + k {
            dp[i] = *nums.last().unwrap();
        }

        for i in (0..n - 1).rev() {
            dp[i] = nums[i] + (1..=k).map(|k| dp[i + k]).max().unwrap();
        }
...
mod tests {
    ...
    fn example1() {
        ...
        assert_eq!(Solution::max_result(nums, 2), 7);
    ...
    fn example2() {
        ...
        assert_eq!(Solution::max_result(nums, 3), 17);
    ...
    fn test_k1() {
        let nums = vec![1, -5, -20, 4, -1, 3, -6, -3];
        let expected = nums.iter().sum();
        assert_eq!(Solution::max_result(nums, 1), expected);
    }

    #[test]
    fn test_1to10000_repeat10_k100000() {
        let nums = (0..100000).map(|n| n % 10000 + 1).collect();
        assert_eq!(Solution::max_result(nums, 1000), 500050000);
    }
}