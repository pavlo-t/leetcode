...
class bw39_1 extends AnyWordSpec with Matchers {
  /**
  ...
  object Solution {
    def decrypt(code: Array[Int], k: Int): Array[Int] = {
      val result = code.clone()
      for (i <- code.indices) {
        result(i) =
          if (k < 0)
            (i + k until i).map(j => if (j < 0) j + code.length else j).map(code).sum
          else if (k > 0)
            (i + 1 to i + k).map(j => if (j >= code.length) j - code.length else j).map(code).sum
          else 0
      }
      result
    }
  ...
  import Solution.decrypt

  "Example 1: (code = [5,7,1,4], k = 3) -> [12,10,16,13]" in {
    decrypt(Array(5, 7, 1, 4), 3) shouldBe Array(12, 10, 16, 13)
  ...
  "Example 2: (code = [1,2,3,4], k = 0) -> [0,0,0,0]" in {
    decrypt(Array(1, 2, 3, 4), 0) shouldBe Array(0, 0, 0, 0)
    //Explanation: When k is zero, the numbers are replaced by 0.
  }
  "Example 3: (code = [2,4,9,3], k = -2) -> [12,5,6,13]" in {
    decrypt(Array(2, 4, 9, 3), -2) shouldBe Array(12, 5, 6, 13)
